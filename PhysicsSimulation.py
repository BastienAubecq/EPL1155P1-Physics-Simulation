...
def simulation():
    ...
    """
    global t, x, v, a, y, e_cin, e_pot, e_tot
...
def plotMovement(recalculate=True):
    ...
    if settings["options"]["[Exp]"]: 
        params = {"x(t)": (x, "blue", xExp, "orange"), "v(t)": (v, "red", vExp, "orange"), "a(t)": (a, "green", aExp, "orange"), "y(t)": (y, "blue", yExp, "orange")}
    else:
        params = {"x(t)": (x, "blue"), "v(t)": (v, "red"), "a(t)": (a, "green"), "y(t)": (y, "blue")}
    for i in range(len(activeOptions)):
        if len(params[activeOptions[i]]) > 2:
            plots.append(createPlot([t, tExp], [params[activeOptions[i]][0], params[activeOptions[i]][2]], [activeOptions[i], activeOptions[i] + " Exp"], (len(activeOptions), 1, i+1), [params[activeOptions[i]][1], params[activeOptions[i]][3]], True))
        else:
            plots.append(createPlot([t], [params[activeOptions[i]][0]], [activeOptions[i]], (len(activeOptions), 1, i+1), [params[activeOptions[i]][1]], True))
...
def plotEnergy(recalculate=True):
    ...
    plots.append(createPlot([t] * len(yValues), yValues, activeOptions, (1, 1, 1), colors, False))
...
settingNames.remove("options")
for i in range(len(settingNames)):
    l = Label(settingsFrame, text=str(settingNames[i]) + " = ", font=("Calibri", 15, "bold"))
    l.place(x=12, y=60+40*i, anchor="w")
    window.update()
    settingsEntries.append(Entry(settingsFrame, width=5, background="white", font=("Calibri", 15, "bold")))
    settingsEntries[-1].insert(0, str(settings[settingNames[i]]))
    settingsEntries[-1].place(x=12+l.winfo_width(), y=60+40*i, anchor="w")
    settingsEntries[-1].bind("<Button-1>", lambda event, id=i: clearEntry(id))
    settingsEntries[-1].bind("<Return>", lambda event, id=i: replot(id))

#Setting Matplotlib canvas and toolbar
figure = Figure(figsize = (6, 5), dpi = 100)
figure.subplots_adjust(top=0.92, bottom=0.08, left=0.1, right=0.9, hspace=0.5)
plots = []
